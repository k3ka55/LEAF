...
{
    public class CSI_Syunc : ICSI_Syunc
    {
        ...
        public void getCSI()
        {
            using (var scope = new TransactionScope())
            {
                STI_PUSH_ENTITY.csiRecords = (from x in CloudDB.Customer_Sale_Indent
                                              where x.DC_Code == dc_Code && x.is_Deleted == false && x.is_Syunc == false && x.CSI_Approved_Flag == true
                ...
                STI_PUSH_ENTITY.CSINumGenClass = (from c in CloudDB.CSI_NUM_Generation
                                                  select new CSINumGen
                                                  {
                                                      CSI_Num_Gen_Id = c.CSI_Num_Gen_Id,
                                                      Financial_Year = c.Financial_Year,
                                                      CSI_Last_Number = c.CSI_Last_Number
                                                  }).ToList();

                int count = STI_PUSH_ENTITY.csiRecords.Count;
                int alcount = STI_PUSH_ENTITY.CSINumGenClass.Count;

                using (var tw = new StreamWriter(Path, true))
                {
                    tw.WriteLine(count + " Records Fetched from CSI and " + alcount + " Auto Generated Numbers. --" + DateTime.Now.ToString("hh:mm tt") + "\n");
                    tw.Close();
                }
        ...
        public void CSISet()
        {
            ...
            if (STI_PUSH_ENTITY.csiRecords != null)
            {
                foreach (var list in STI_PUSH_ENTITY.csiRecords)
                {
                    ...
                    {
                        CSI_Sync_Another csAnother = new CSI_Sync_Another();
