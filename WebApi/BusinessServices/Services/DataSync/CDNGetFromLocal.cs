...
{
    public class CDNGetFromLocal : ICDNGetFromLocal
    {
        ...
        public void CDNSet()
        {
            ...
            if (STI_PUSH_ENTITY.stnRecordes != null)
                foreach (var disp in STI_PUSH_ENTITY.stnRecordes)
                {
                    ...
                    {
                        CDNSetToLive csAnother = new CDNSetToLive();
        ...
        public void CDNGet()
        {
            using (var scope = new TransactionScope())
            {
                STI_PUSH_ENTITY.stnRecordes = (from x in CloudDB.Dispatch_Creation
                                               where x.Dispatch_Type == "Customer" && x.Customer_Dispatch_Number != null && x.is_Syunc == false && x.is_Deleted == false
                ...
                using (var tw = new StreamWriter(Path, true))
                {
                    tw.WriteLine(count + " Records Fetched from CDN and " + alcount + " Auto Generated Numbers. --" + DateTime.Now.ToString("hh:mm tt") + "\n");
                    tw.Close();
                }
