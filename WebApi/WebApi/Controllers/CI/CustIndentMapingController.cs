...
namespace WebApi.Controllers.CI
{
    ...
    public class CustIndentMapingController : ApiController
    {
        private readonly ICustIndentMapingService _mappServices;

        #region Public Constructor

        public CustIndentMapingController()
        {
            _mappServices = new CustIndentMapingService();
        }
        ...
        public HttpResponseMessage Get(int id)
        {
            ...
            string message = "";
            Customer_CustomerIndetMap_Entity rs = new Customer_CustomerIndetMap_Entity();
            if (id != null)
            {
        ...
        [HttpPost]
        public HttpResponseMessage CreateIndentMapping(Customer_CustomerIndetMap_Entity mapp)
        {
        ...
        [HttpGet]
        public HttpResponseMessage SearchMapping(int? roleId, string region, string location, string dccode, string Url)
        {
            var list = _mappServices.searchMapping(roleId, region, location, dccode,Url);

            return Request.CreateResponse(list);
        }

        [HttpPost]
        public HttpResponseMessage UpdateMapping([FromBody]Customer_CustomerIndetMap_Entity mapp)
        {

            ResponseEntity response = new ResponseEntity();
            bool responseCode;
            if (mapp.CRT_Mapping_ID > 0)
            {
                responseCode = _mappServices.UpdateCustomerIndentMapping(mapp.Template_ID, mapp);
                if (responseCode)
                {
        ...
        [HttpGet]
        public HttpResponseMessage DeleteIndentMapping(int id)
        {
